...
from catalog.views import BooksLoanedByUserListView, AllBorrowedBooksListView
    ...
    def setUpTestData(cls):
        ...
        for author in range(number_of_authors):
            Author.objects.create(
                first_name=f"Peter {author}", last_name=f"Parker {author}"
            )
...
class RenewBookInstancesViewTest(TestCase):
    def setUp(self):
        ...
        # Create a book
        test_author = Author.objects.create(first_name="Bruce", last_name="Banner")
        test_genre = Genre.objects.create(name="Fantasy")
        test_language = Language.objects.create(name="en")
        test_book = Book.objects.create(
            title="Book Title",
    ...
    def test_logged_in_with_perm_borrowed_book(self):
        login = self.client.login(username="testuser2", password="b07d8dcfce150246e")
        res = self.client.get(
    ...
    def test_HTTP404_invalid_book(self):
        test_uid = uuid.uuid4()
        login = self.client.login(username="testuser2", password="b07d8dcfce150246e")
        res = self.client.get(reverse("renew-book-librarian", kwargs={"pk": test_uid}))
    ...
    def test_renew_form_initially_date_three_weeks_in_future(self):
        ...
        self.assertEqual(res.context["form"].initial["due_back"], three_weeks_in_future)
    ...
    def test_form_invalid_date_in_past(self):
        ...
        login = self.client.login(username="testuser2", password="b07d8dcfce150246e")
        res = self.client.post(self.renew_url_valid_book, {"due_back": date_in_past})
        self.assertEqual(res.status_code, 200)
        self.assertFormError(res, "form", "due_back", "Invalid date - Renewal đã qua")

    def test_form_invalid_date_in_future(self):
        invalid_date_in_future = datetime.date.today() + datetime.timedelta(weeks=5)
        login = self.client.login(username="testuser2", password="b07d8dcfce150246e")
        res = self.client.post(
            self.renew_url_valid_book, {"due_back": invalid_date_in_future}
        )
        self.assertEqual(res.status_code, 200)
        self.assertFormError(
            res, "form", "due_back", "Invalid date - Renewal more than 4 weeks ahead"
        )


@pytest.mark.django_db
class TestLoanedBooksByUserListView:
    def test_loaned_books_authenticated_user(self):
        path = reverse("my-borrowed")
        request = RequestFactory().get(path)
        request.user = mixer.blend(User)

        view = BooksLoanedByUserListView()
        view.setup(request)
...
@pytest.mark.django_db
class TestAllBorrowedBooksListView(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.path = reverse("all-borrowed")
        cls.factory = RequestFactory()
        cls.view = AllBorrowedBooksListView()

    def test_with_authenticated_user(self):
        request = self.factory.get(self.path)
        request.user = mixer.blend(User)
        self.view.setup(request)
        response = self.view.get(self.path)
        assert response.status_code == 200
    ...
    def test_with_unauthenticated_user(self):
        ...
        response = self.view.get(self.path)
        assert response.status_code == 200
        assert "catalog/all_borrowed_books.html" in response.template_name
